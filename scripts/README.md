...
| `build_site.py` | Writes the menu, footer and hashed asset URLs into all four pages, the figures, the tool table from the MCP registry, and the four demos from allowlisted data (anchors, the help directory, the commitment ledger, the scorecard, the recorded examples). Every displayed quote must be its claim's registered quote, located in the captured source; a ledger with a total never reaches a page. `--check` gates it. | `make build-site` |
